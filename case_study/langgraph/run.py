"""Run the shared research-assistant task with LangGraph orchestration."""
...
def main() -> int:
    ...
    run_id = args.run_id or f"langgraph-{variant.value}"
    budget = Budget(max_steps=args.max_steps) if args.max_steps is not None else None
    state = asyncio.run(
        LangGraphCaseStudy(model_factory=_model_factory(args)).run(
            variant,
            run_id=run_id,
            resume=args.resume,
